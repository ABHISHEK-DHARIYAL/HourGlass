// Purpose: Custom hook managing schedule checking, notification timers, web audio alerts, and system notification triggers
/**
 * @license
...
export function useNotifications({
  ...
  // Keep settings synchronized, clear previous notifications, and sync FCM token on UID change
  useEffect(() => {
    setActiveNotifications([]);
    const loadedSettings = loadNotificationSettings(userId);
    setSettings(loadedSettings);

    if (userId && userId !== 'guest_user') {
      // Register/sync FCM token for the new authenticated UID
      saveNotificationSettings(userId, loadedSettings).catch(err => {
        console.warn('Failed to auto-sync FCM token on UID change:', err);
      });
    }
  }, [userId]);
