// Purpose: Daily habit tracker section with 7-day completion matrix, streak counters, and habit creation dialog
/**
 * @license
...
export default function HabitTrackerSection({
  ...
  return (
    <div className="surface-card p-4 flex flex-col gap-4 animate-fade-up">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-ledger-paper">Daily Rituals</h3>
            <p className="text-[11px] text-ledger-paper-dim tracking-wide mt-0.5">
        ...
        {isAdding && (
          ...
          >
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] text-ledger-paper-dim uppercase font-semibold">Habit Title</label>
              <input
                type="text"
                required
      ...
      {/* Habits List */}
      {habits.length === 0 ? (
        <div className="text-center py-6 px-4 surface-panel rounded-xl">
          <span className="text-xl">✨</span>
          <p className="text-[11px] text-ledger-paper-dim mt-1.5">
            No habits configured yet
          </p>
          <p className="text-[10px] text-ledger-paper-dim/70 mt-1">
        ...
        <div className="flex flex-col gap-3">
          {habits.map((habit) => {
            ...
            return (
              <div
                key={habit.id}
                className="p-3 surface-panel hover:border-ledger-coral/40 rounded-xl flex flex-col gap-3 group relative overflow-hidden transition-all"
              >
                ...
                <div className="pl-1.5 flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <span className="font-bold text-xs text-ledger-paper block leading-tight">
                      {habit.title}
                    </span>
                ...
                {/* 7-day tracker row */}
                <div className="grid grid-cols-7 gap-1 bg-ledger-dark/30 border border-ledger-line p-1.5 rounded-lg">
                  {weekDates.map((dStr) => {
