// Purpose: Monthly calendar grid view component showing daily task densities and navigation
/**
 * @license
...
export default function MonthView({ currentDateStr, tasks, exceptions = [], onSelectDate }: MonthViewProps) {
  ...
  return (
    <div className="w-full surface-card p-5 flex flex-col">
      {/* Month Selection Header */}
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-2xl font-extrabold text-ledger-paper tracking-tight flex items-baseline gap-2">
          {monthNames[month]} <span className="text-base font-normal text-ledger-paper-dim">{year}</span>
        </h2>
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg surface-panel text-xs font-semibold tracking-wide text-ledger-coral hover:border-ledger-coral transition-all cursor-pointer"
          >
            Today
          </button>
      ...
      <div className="grid grid-cols-7 text-center mb-2">
        {weekdays.map((day) => (
          <span
            key={day}
            className="text-[10px] text-ledger-paper-dim uppercase tracking-widest font-bold py-1"
          >
            {day}
          </span>
        ))}
        ...
        {days.map(({ dateStr, dayNum, isCurrentMonth }, idx) => {
          ...
          return (
            <button
              ...
              className={`
                aspect-square relative rounded-xl flex flex-col items-center justify-center transition-all duration-150 cursor-pointer group
                ${isSelected ? 'bg-ledger-coral text-[#0a0c10] font-bold' : 'border border-transparent'}
            ...
            >
              <span
                className={`
                  text-sm font-mono font-medium flex items-center justify-center
                  ${isToday && !isSelected ? 'w-6 h-6 rounded-full text-ledger-coral' : ''}
                `}
                style={isToday && !isSelected ? { boxShadow: 'inset 0 0 0 1.5px var(--color-ledger-coral)' } : undefined}
              >
                {dayNum}
              </span>
              ...
              {hasTasks && (
                <span
                  className={`
                    absolute bottom-1.5 w-1.5 h-1.5 rounded-full transition-transform group-hover:scale-125
                    ${isSelected ? 'bg-[#0a0c10]' : 'bg-ledger-gold'}
                  `}
                />
              )}
            </button>
