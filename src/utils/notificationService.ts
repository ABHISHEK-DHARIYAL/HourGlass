// Purpose: Notification service handling web system notifications, settings persistence, and schedule analysis
...
import { playNotificationSound } from './soundSynthesizer';

export type { NotificationSettings, NotificationAction, RichNotification };
export { DEFAULT_NOTIFICATION_SETTINGS, playNotificationSound };
...
/**
 * Clear FCM token association on backend for user logout isolation
 */
export async function clearFcmTokenOnServer(userId: string): Promise<boolean> {
  if (!userId || userId === 'guest_user') return true;
  try {
    const response = await fetch('/api/clear-fcm-token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.error('Failed to clear FCM token on server:', err);
    return false;
  }
}
...
export function checkForActiveNotifications(
  ...
  const dateStr = now.toLocaleDateString('en-CA', { timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone });
  ...
  const morningTimeStr = settings.morningSummaryTime;
  const eveningTimeStr = settings.eveningSummaryTime;
  ...
  if (settings.habitRemindersEnabled) {
    habits.forEach(habit => {
      ...
      if (!isDone) {
        const habitTimeCheck = currentHour === 14 && currentMinute === 30;
        if (habitTimeCheck) {
